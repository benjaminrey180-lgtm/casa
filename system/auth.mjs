...
// Límite simple en memoria: 10 intentos fallidos por IP+correo cada 15 minutos.
const failures = new Map();
const WINDOW = 15 * 60 * 1000, MAX_FAILURES = 10;
...
export async function login({email, password}, ip) {
 email = String(email || '').trim().toLowerCase();
 const key = `${ip}|${email}`;
 if (limited(key)) throw Object.assign(Error('Demasiados intentos. Espera 15 minutos.'), {status: 429});
 ...
 if (!user || !ok) { fail(key); throw Object.assign(Error('Correo o contraseña incorrectos.'), {status: 401}); }
 failures.delete(key);
